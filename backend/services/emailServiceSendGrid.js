...
if (process.env.SENDGRID_API_KEY) {
  ...
  console.warn("⚠️  SENDGRID_API_KEY not found, email service will not work");
}
...
const sendOrderConfirmationEmail = async (order, language = "vi") => {
  try {
    ...
    console.error("❌ Error sending order confirmation email via SendGrid:");
  ...
  try {
    ...
    const msg = {
      ...
      html: `
        ...
        <p><a href="https://dashboard.render.com" style="background-color: #667eea; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block; margin-top: 10px;">${
    ...
    console.error("❌ Error sending admin notification email via SendGrid:");
    console.error("   Error message:", error.message);
    console.error("   Error code:", error.code);
