...
const createPaymentQR = async (req, res) => {
  try {
    ...
    const newPaymentQR = new PaymentQR({ bankName, accountNumber, accountName, qrCodeImage, translations, isActive, displayOrder });
...
const updatePaymentQR = async (req, res) => {
  try {
    const { bankName, accountNumber, accountName, qrCodeImage, translations, isActive, displayOrder } = req.body;
    const updatedPaymentQR = await PaymentQR.findByIdAndUpdate(req.params.id, { bankName, accountNumber, accountName, qrCodeImage, translations, isActive, displayOrder }, { new: true, runValidators: true });
    if (!updatedPaymentQR) {
      return res.status(404).json({ message: "Không tìm thấy thông tin QR thanh toán" });
    }
...
module.exports = { getAllPaymentQRs, getActivePaymentQRs, getPaymentQRById, createPaymentQR, updatePaymentQR, deletePaymentQR, toggleActiveStatus };
